...
ssize_t read(int fd, void *buf, size_t nbyte) {
  ...
  // 需要提醒的是，该fd会在accept函数或者socket函数时，创建alloc_by_fd(),
  // 并将其fd信息放入数组
  rpchook_t *lp = get_by_fd(fd);

  // 如果该fd不是被hook得到的，或者用户主动的把它设置成了非阻塞,
  // 则直接调用原生的read，而不再hook 换句话说:
...
ssize_t recvfrom(int              socket,
  ...
  if (!co_is_enable_sys_hook()) {
    return g_sys_recvfrom_func(socket,
                               buffer,
                               length,
                               flags,
                               address,
...
struct hostent *co_gethostbyname(const char *name) {
  ...
  int ret = -1;
  while (ret = gethostbyname_r(name,
                               host,
