...
public:
  ...
  inline void *get(int fd) {
    int idx = fd % row_size;
    if (id < 0 || idx >= sizeof(m_pp) / sizeof(m_pp[0])) {
      return NULL;
    }
...
int co_epoll_del(int epfd, int fd) {
  struct timespec       t      = {0};
  struct kevent_pair_t *events = (struct kevent_pair_t *)get_fd_map()->get(fd);
...
int co_epoll_ctl(int epfd, int op, int fd, struct epoll_event *ev) {
  ...
  do {
    ...
    if (EPOLLOUT & ev->events) {
      struct kevent kev = {0};
      EV_SET(&kev, fd, EVFILT_WRITE, 0, 0, ptr);
      ret = kevent(fd, &kev, 1, NULL, 0, &t);
      if (ret) {
        break;
      }
    }
...
struct co_epoll_res_alloc(int n) {
  struct co_epoll_res *ptr =
      (struct co_epoll_res *)malloc(sizeof(struct co_epoll_res));

  ptr->size = n;
  ptr->events =
      (struct co_epoll_event *)calloc(1, n * sizeof(struct co_epoll_event));
